...
$mikrotik_script = "/interface sstp-client add connect-to=$domain disabled=no name=SaaS-VPN password={$client['vpn_password']} profile=default-encryption user={$client['username']}
...
<head>
    ...
    <style>
        ...
        pre { background: #333; color: #0f0; padding: 15px; border-radius: 5px; overflow-x: auto; font-family: monospace; }
...
<body>
    <div class="container">
        ...
        <div class="box">
            <h3>🔗 Étape 1 : Connecter votre MikroTik</h3>
            <p>Copiez le code ci-dessous et collez-le dans le <b>New Terminal</b> de votre Winbox pour relier votre routeur à la plateforme :</p>
            <pre><?= htmlspecialchars($mikrotik_script) ?></pre>
            <p><small><i>Une fois collé, le routeur sera connecté avec l'adresse IP interne : <b><?= $client['vpn_ip'] ?></b></i></small></p>
        </div>
